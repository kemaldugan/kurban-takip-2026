...
// Yöneticinin WhatsApp numarası (başında 90, boşluksuz)
const WA_YONETICI = "905552553456"; // ← kendi numaranızla değiştirin

/* WhatsApp mesaj gönder */
function waGonder(telefon, mesaj) {
  const numara = telefon.replace(/\D/g,"");
  const tam = numara.startsWith("0") ? "9"+numara : numara.startsWith("90") ? numara : "90"+numara;
  const url = `https://wa.me/${tam}?text=${encodeURIComponent(mesaj)}`;
  window.open(url, "_blank");
}

/* ════ ÖDEME INPUT BİLEŞENİ ════ */
function OdemeInput({tutar, odenen, onChange}) {
  const [deger, setDeger] = useState(
    odenen ? Number(odenen).toLocaleString("tr-TR") : ""
  );
  const [odak, setOdak] = useState(false);

  // Dışarıdan güncelleme gelirse senkronize et
  useEffect(() => {
  ...
  return (
    <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap"}}>
      <input
        ...
        onFocus={() => { setOdak(true); setDeger(odenen ? String(odenen) : ""); }}
        onBlur={() => {
          setOdak(false);
      ...
      />
      <span style={{fontSize:10,color:"#9a6640",whiteSpace:"nowrap"}}>TL → odaktan çık</span>
    </div>
  );
}
...
const CSS = `
  *,*::before,*::after{box-sizing:border-box;}
  ...
  .kayan{display:inline-block;white-space:nowrap;animation:kayDir 42s linear infinite;color:#7a3e10;font-family:${FONT};font-size:13px;letter-spacing:.8px;}
  
  @media(hover:hover){.hkart:hover{transform:translateY(-3px);box-shadow:0 10px 30px rgba(122,62,16,.15);}}
  ...
  /* Sekme bar */
  .sbar{display:flex;overflow-x:auto;-webkit-overflow-scrolling:touch;border-bottom:2px solid rgba(200,134,26,.2);scrollbar-width:none;}
  ...
  .modal-kart{background:linear-gradient(160deg,#fdf6ec,#f7ede0);border:1px solid #c8861a;border-radius:18px;width:100%;max-width:480px;max-height:94vh;overflow-y:auto;}
  @media(max-width:500px){.modal-kart{border-radius:14px;max-height:97vh;}}
  .talep-kart{background:linear-gradient(160deg,#fdf6ec,#f7ede0);border:1px solid #c8861a;border-radius:14px;padding:22px;width:100%;max-width:380px;}
  ...
  /* Güncelleme formu */
  .gform{display:flex;flex-direction:column;gap:10px;padding:14px;background:rgba(200,134,26,.08);border-radius:10px;margin-top:12px;border:1px solid rgba(200,134,26,.25);}
  ...
  .foto-yukle-alani{border:2px dashed rgba(200,134,26,.5);border-radius:12px;padding:20px;text-align:center;cursor:pointer;transition:border-color .2s,background .2s;position:relative;overflow:hidden;}
  .foto-yukle-alani:hover,.foto-yukle-alani:active{border-color:#c8861a;background:rgba(200,134,26,.1);}
...
function FotoYukle({mevcut, onChange}) {
  ...
  return (
    <div>
      <label style={{fontSize:12,color:"#8a5c30",display:"block",marginBottom:6}}>📷 Fotoğraf</label>
          ...
          : <div style={{pointerEvents:"none"}}>
              {yukleniyor
                ? <div>
                    ...
                    <div style={{fontSize:13,color:"#c8861a"}}>Yükleniyor...</div>
                  </div>
                : <div>
                    <div style={{fontSize:32,marginBottom:6}}>📷</div>
                    <div style={{fontSize:13,color:"#7a3e10",fontWeight:600}}>Fotoğraf Çek / Seç</div>
                    <div style={{fontSize:11,color:"#6b4423",marginTop:3}}>Dokunun — kamera veya galeri</div>
                  </div>
              }
            </div>
        }
...
function KayanBant() {
  ...
  return (
    <div style={{background:"linear-gradient(90deg,#f0e6d3,#e8d8c0,#f0e6d3)",borderTop:"1px solid rgba(200,134,26,.3)",borderBottom:"1px solid rgba(200,134,26,.3)",padding:"8px 0",overflow:"hidden"}}>
...
export default function App() {
  ...
  const onayla = (tid, ekBilgi={})=>{
    ...
    /* Hissedara WhatsApp onay mesajı */
    const hBilgi2 = hRef.current.find(x=>x.id===t.hayvanId);
    const teslimTxt = {ev:"Eve teslim",arac:"Araçla teslim",ciftlik:"Çiftlikte teslim",diger:"Belirtilen adrese",belirtilmedi:"Detay için irtibata geçilecek"};
    const waOnay = `بِسْمِ اللَّهِ الرَّحْمَنِ الرَّحِيمِ

✅ *HİSSE ONAYLANDI*

Sayın ${t.ad},

Kurban hisseniz onaylanmıştır.

🐄 Hayvan: #${hBilgi2?.numara}
💰 Hisse bedeli: ${new Intl.NumberFormat("tr-TR",{style:"currency",currency:"TRY",maximumFractionDigits:0}).format(t.tutar)}
📋 Vekalet: ${ekBilgi.vekalet?"Alındı ✓":"Alınmadı — lütfen iletişime geçin"}
🚚 Teslimat: ${teslimTxt[ekBilgi.teslimat]||"Belirtilmedi"}${ekBilgi.not?"
📝 Not: "+ekBilgi.not:""}

Hayırlı kurbanlar olsun 🌙
Murat Yalvaç Öğrenci Yurdu`;
    setTimeout(() => waGonder(t.telefon, waOnay), 400);
  };
  ...
  const talepGonder = async (t)=>{
    ...
    /* Yöneticiye WhatsApp bildirimi */
    const hBilgi = hRef.current.find(x=>x.id===t.hayvanId);
    const waMesaj = `🐄 *YENİ HİSSE TALEBİ*

👤 Ad: ${t.ad}
📞 Tel: ${t.telefon}
🐄 Hayvan: #${hBilgi?.numara} (${hBilgi?.tip==="buyukbas"?"Büyükbaş":"Küçükbaş"})
💰 Tutar: ${new Intl.NumberFormat("tr-TR",{style:"currency",currency:"TRY",maximumFractionDigits:0}).format(t.tutar)}
🕐 ${new Date().toLocaleString("tr-TR")}

📋 Onaylamak için yönetici paneline giriş yapın.`;
  ...
  return (
    <>
      <style>{CSS}</style>
      {sbHata&&<div style={{background:"#7f1d1d",color:"#7a1a1a",fontSize:11,textAlign:"center",padding:"5px",fontFamily:FONT}}>⚠️ Bağlantı sorunu — veriler geçici olarak çevrimdışı saklanıyor</div>}
...
function PublicPanel({hayvanlar,talepler,onTalep,onAdmin}) {
  ...
  return (
    <div style={S.page}>
      {/* HEADER */}
      <div style={{background:"linear-gradient(180deg,#7a1a1a,#5a0f0f)",borderBottom:"3px solid #c8861a",position:"relative",padding:"20px 16px 18px",textAlign:"center"}}>
        ...
        <h1 style={{margin:"4px 0 2px",fontSize:"clamp(16px,4.5vw,26px)",fontWeight:700,color:"#fff8f0",letterSpacing:"clamp(1px,0.5vw,3px)",fontFamily:FONT}}>KURBAN ORGANİZASYONU</h1>
              ...
              return (
                <div key={h.id} className="hkart" onClick={()=>setDetay(h)} style={{...S.card,overflow:"hidden"}}>
                  {h.foto
                    ...
                    :<div style={{height:80,background:"rgba(200,134,26,.07)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:38}}>{h.tip==="buyukbas"?"🐄":h.kategori==="koyun"?"🐑":"🐐"}</div>
                  }
                  <div style={{padding:"10px 12px"}}>
                    ...
                    <div style={{fontSize:"clamp(15px,4vw,19px)",fontWeight:700,color:"#fff8f0"}}>{formatTL(hT)}<span style={{fontSize:10,fontWeight:400,color:"#6b4423"}}>/hisse</span></div>
          ...
          <div className="modal-bg" onClick={()=>setDetay(null)}>
            <div className="modal-kart" onClick={e=>e.stopPropagation()}>
              {h.foto
                ...
                :<div style={{height:80,background:"rgba(200,134,26,.07)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:48,borderRadius:"18px 18px 0 0"}}>{h.tip==="buyukbas"?"🐄":h.kategori==="koyun"?"🐑":"🐐"}</div>
              }
              <div style={{padding:"16px 18px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  <div>
                    ...
                    <h2 style={{margin:"0 0 1px",color:"#7a3e10",fontSize:"clamp(17px,4.5vw,22px)",fontFamily:FONT}}>#{h.numara} Nolu Hayvan</h2>
                ...
                {h.aciklama&&(
                  <div style={{background:"#f7f0e5",border:"1px solid rgba(212,160,23,.12)",borderRadius:8,padding:"9px 12px",marginBottom:12}}>
                ...
                {efBos>0
                  ?<button onClick={()=>{setDetay(null);setTalepModal(h);}} style={{width:"100%",padding:"13px",background:"linear-gradient(90deg,#8b1a1a,#c0392b)",border:"none",borderRadius:10,color:"#fff8f0",cursor:"pointer",fontFamily:FONT,fontWeight:700,fontSize:"clamp(13px,3.5vw,15px)",touchAction:"manipulation"}}>
      ...
      {talepModal&&(
        <div className="modal-bg" onClick={()=>setTalepModal(null)}>
          <div className="talep-kart" onClick={e=>e.stopPropagation()}>
            <h3 style={{margin:"0 0 3px",color:"#7a3e10",fontFamily:FONT,fontSize:"clamp(15px,4vw,18px)"}}>🐄 Hisse Talebi</h3>
            ...
            <div style={{background:"rgba(212,160,23,.07)",border:"1px solid rgba(200,134,26,.18)",borderRadius:8,padding:"9px 12px",marginBottom:14,fontSize:"clamp(11px,3vw,13px)",color:"#c8861a"}}>
...
function LoginPanel({sifre,setSifre,err,onGiris,onGeri}) {
  return (
    <div style={{...S.page,display:"flex",alignItems:"center",justifyContent:"center",minHeight:"100vh",padding:16}}>
      <div style={{background:"#ffffff",border:"1px solid #c8861a",borderRadius:18,padding:"clamp(24px,6vw,36px) clamp(18px,5vw,28px)",width:"100%",maxWidth:330,textAlign:"center"}}>
    ...
    <div style={S.page}>
      ...
      <div style={{background:"linear-gradient(90deg,#6b1515,#8b1a1a,#6b1515)",borderBottom:"2px solid #c8861a",padding:"10px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",gap:8}}>
        <div style={{minWidth:0}}>
          <h1 style={{margin:0,fontSize:"clamp(13px,3.5vw,17px)",color:"#7a3e10",fontFamily:FONT}}>⚙️ Yönetici Paneli</h1>
          ...
          <div>
                  ...
                  <div key={t.id} style={{...S.card,padding:"13px 14px",marginBottom:12}}>
                    ...
                    <div style={{marginBottom:10}}>
                      <div style={{display:"flex",justifyContent:"space-between",flexWrap:"wrap",gap:4,marginBottom:4}}>
                        <span style={{color:"#7a3e10",fontWeight:700,fontSize:14}}>#{h?.numara} nolu hayvan</span>
                      ...
                      <div style={{display:"flex",gap:12,fontSize:12,color:"#8a5c30",flexWrap:"wrap"}}>
                        <span>📞 {t.telefon}</span>
                        <span style={{color:"#c8861a"}}>💰 {formatTL(t.tutar)}</span>
                      </div>
                    </div>
                    ...
                    <div style={{background:"rgba(200,134,26,.07)",border:"1px solid rgba(200,134,26,.2)",borderRadius:9,padding:"11px",marginBottom:10}}>
                      <p style={{margin:"0 0 9px",fontSize:11,color:"#7a3e10",fontWeight:700}}>📋 Onay Bilgileri</p>
            ...
            {talepler.filter(t=>t.durum!=="bekliyor").length===0
              ...
              :talepler.filter(t=>t.durum!=="bekliyor").slice().reverse().map(t=>{
                const h=hayvanlar.find(x=>x.id===t.hayvanId);
                return (
                  <div key={t.id} style={{background:"#faf4eb",border:"1px solid rgba(255,255,255,.06)",borderRadius:8,padding:"8px 11px",marginBottom:5,fontSize:12}}>
                ...
                <div key={h.id} style={{...S.card,padding:"12px 13px",marginBottom:11,border:`1px solid ${h.durum==="aktif"?"rgba(212,160,23,.25)":"rgba(255,0,0,.2)"}`}}>
                    ...
                    <div style={{display:"flex",gap:9,alignItems:"center",flex:1,minWidth:0}}>
                      ...
                      <div style={{minWidth:0}}>
                        <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                          <span style={{fontSize:15,color:"#7a3e10",fontWeight:700}}>#{h.numara}</span>
                  ...
                  {isAcik&&(
                    <div style={{marginTop:12,paddingTop:12,borderTop:"1px solid rgba(212,160,23,.12)"}}>
                      ...
                      {/* HİSSEDAR EKLE */}
                      <div style={{background:"rgba(200,134,26,.08)",border:"1px solid rgba(200,134,26,.18)",borderRadius:9,padding:"12px",marginBottom:12,marginTop:isDuz?12:0}}>
                        <p style={{margin:"0 0 10px",fontSize:12,color:"#7a3e10",fontWeight:700}}>➕ Hissedar Ekle <span style={{fontSize:10,fontWeight:400,color:"#6b4423"}}>({bos} boş yer)</span></p>
                        {/* Zorunlu alanlar */}
                        <div className="hfrow" style={{marginBottom:8}}>
                      ...
                      {h.hisseler.filter(x=>x.durum==="onaylı").length===0
                        ?<p style={{color:"#9a6640",fontSize:11}}>Henüz onaylı hisse yok.</p>
                        :h.hisseler.filter(x=>x.durum==="onaylı").map((hisse,i)=>{
                          ...
                          return (
                          <div key={hisse.id} style={{background:"#fdf8f2",border:"1px solid rgba(255,255,255,.07)",borderRadius:10,padding:"10px 12px",marginBottom:8}}>
                            {/* Üst satır: isim + sil */}
                            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
                              <div>
                                <span style={{color:"#7a3e10",fontWeight:700,fontSize:13}}>{i+1}. {hisse.ad}</span>
                                <span style={{fontSize:11,color:"#6b4423",marginLeft:8}}>📞 {hisse.telefon}</span>
                              </div>
                            ...
                            {hisse.not&&<div style={{fontSize:11,color:"#8a5c30",marginBottom:8,fontStyle:"italic"}}>📝 {hisse.not}</div>}
                            {/* Ödeme takibi */}
                            <div style={{background:"rgba(200,134,26,.08)",borderRadius:8,padding:"8px 10px"}}>
                              <div style={{display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:6,flexWrap:"wrap",gap:4}}>
                                <span style={{color:"#2d1a08"}}>Toplam: <strong style={{color:"#c8861a"}}>{formatTL(hisse.tutar)}</strong></span>
                              ...
                              <OdemeInput
                                tutar={hisse.tutar}
                                odenen={hisse.odenen||0}
                                onChange={(v)=>onOdeme(h.id,hisse.id,v)}
                              />
                            </div>
                          </div>
                        );})
          ...
          <div style={{maxWidth:420}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:11}}>
              ...
              <div>
                ...
                <p style={{margin:"3px 0 0",fontSize:10,color:"#9a6640"}}>Sadece rakam girin, nokta otomatik eklenir</p>
              </div>
              {yeniH.tip==="buyukbas"&&(
              ...
              {yeniH.fiyat&&+yeniH.maxHisse>0&&(
                <div style={{background:"rgba(200,134,26,.1)",border:"1px solid rgba(200,134,26,.2)",borderRadius:8,padding:"9px 13px",fontSize:13,color:"#c8861a"}}>
            ...
            <div>
              ...
              <div style={{...S.card,padding:"13px 15px",marginBottom:16}}>
                <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#8a5c30",marginBottom:6}}>
                  ...
                  <span style={{color:"#7a3e10",fontWeight:700}}>%{oran} — {dm}/{tm}</span>
                </div>
              ...
              <div style={{...S.card,padding:"13px 14px",marginBottom:16,overflowX:"auto"}}>
                <p style={{margin:"0 0 9px",fontSize:12,color:"#7a3e10",fontWeight:700}}>Hayvan Bazlı Doluluk</p>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:300}}>
                  <thead>
                    <tr style={{borderBottom:"1px solid rgba(200,134,26,.2)"}}>
                  ...
                  <tbody>
                    {[...hayvanlar].sort((a,b)=>a.kesimSirasi-b.kesimSirasi).map(h=>{
                      ...
                      return (
                        <tr key={h.id} style={{borderBottom:"1px solid rgba(200,134,26,.1)"}}>
                          <td style={{padding:"6px 7px",color:"#6b4423"}}>{h.kesimSirasi}.</td>
                          <td style={{padding:"6px 7px",color:"#7a3e10",fontWeight:700}}>#{h.numara}</td>
              ...
              <div style={{...S.card,padding:"14px",marginBottom:14}}>
                <p style={{margin:"0 0 5px",fontSize:12,color:"#7a3e10",fontWeight:700}}>📥 Excel / CSV İndir</p>
